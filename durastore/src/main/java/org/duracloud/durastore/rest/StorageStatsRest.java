...
public class StorageStatsRest extends BaseRest {
    ...
    public Response getSpaceStatsOverTime(@PathParam("spaceID") String spaceId,
        ...
        try {
            ...
            return handleException(e, MessageFormat.format("error getting storage stats, {0}:{1}:{2} [{3}:{4}]",
                                                           account, storeId, spaceId, startMs, endMs));
        }
    }
    ...
    public Response getStoreStatsOverTime(@QueryParam("storeID") String storeId,
        ...
        try {
            ...
            return handleException(e, MessageFormat.format("error getting storage stats, {0}:{1} [{3}:{4}]",
                                                           account, storeId, startMs, endMs));
        }
    }
    ...
    private GroupBy getGroupBy(String groupBy) {
        if (groupBy == null) {
            ...
            try {
                ...
                String message = groupBy + " is not a valid value for the groupBy parameter. You must specify " +
                                 "one of the following values: " + StringUtils.join(GroupBy.values(), ",");
